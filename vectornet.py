...
class SubGraph(nn.Module):
    def __init__(self, in_channels, sub_graph_layers, hidden_state):
        ...
        for i in range(sub_graph_layers):
            ...
            in_channels *= 2
        self.bach_norm = nn.BatchNorm1d(in_channels)
    ...
    def forward(self, data):
        ...
        result.x = self.bach_norm(result.x)
...
class VectorNet(nn.Module):
    def __init__(
        ...
        self.self_attention_layer = SelfAttention(
            self.poly_feature_shape, global_graph_width
        )
        self.pred_traj_mlp = PredTrajMLP(
    ...
    def forward(self, data):
        ...
        pred = self.pred_traj_mlp(self_attention_out[:, [0]].squeeze(1))
        return pred
