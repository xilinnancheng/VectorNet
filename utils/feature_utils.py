...
def trans_gt_offset_format(gt):
    ...
    # pdb.set_trace()
    assert (
        offset_gt.cumsum(axis=0) - gt
    ).sum() < 1e-6, f"{(offset_gt.cumsum(axis=0) -gt).sum()}"

    return offset_gt


def encode_features(agent_feature, obj_feature_ls, lane_feature_ls):
    """
    args:
        agent_feature_ls:
    ...
    agent_nd = np.hstack(
        (
            agent_feature[0],
            np.ones((agent_len, 1)),
            agent_feature[2].reshape((-1, 1)),
            np.ones((agent_len, 1)) * polyline_id,
        )
    )
    ...
    # id --> [prev_traj_len,  curr_traj_len]
    ...
    for obj_feature in obj_feature_ls:
        ...
        obj_nd = np.hstack(
            (
                obj_feature[0],
                np.zeros((obj_len, 1)),
                obj_feature[2].reshape((-1, 1)),
                np.ones((obj_len, 1)) * polyline_id,
            )
        )
    ...
    data = [
        [
            polyline_features.astype(np.float32),
            offset_gt,
            traj_id2mask,
            lane_id2mask,
            traj_nd.shape[0],
...
def save_features(df, name, dir_=None):
    ...
    df.to_pickle(os.path.join(dir_, name))
